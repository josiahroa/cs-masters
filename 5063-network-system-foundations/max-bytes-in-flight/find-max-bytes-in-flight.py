...
def findMaxBytesInFlight(pcapfile):   
   ...
   for pkt in pcap:
        ...
        if isFlowEgress(pkt, flow):
            ...
            if seq_num + data_len > flow.highestSeqNum:
               flow.highestSeqNum = seq_num + data_len
               flow.pktLenOfHighestSeqNumPacket = data_len

            bytes_in_flight = flow.highestSeqNum - flow.ackNumReceived
