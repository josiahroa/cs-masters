...
class Router:
    ...
    def update(self, rt):
        prefix_str = rt.pfx_str()

        if prefix_str not in self.rib:
            self.rib[prefix_str] = []

        for i, existing_route in enumerate(self.rib[prefix_str]):
            if existing_route.neighbor == rt.neighbor:
                self.rib[prefix_str][i] = rt
                return

        self.rib[prefix_str].append(rt)
        return
    ...
    def withdraw(self, rt):
        ...
        self.rib[prefix_str] = [route for route in self.rib[prefix_str] 
                                if route.neighbor != rt.neighbor]
        
        if not self.rib[prefix_str]:
            del self.rib[prefix_str]
    ...
    def next_hop(self, ipaddr):
        ...
        for prefix_str in self.rib:
            prefix, length = prefix_str.split("/")
            length = int(length)
            prefix_binary = self.convertToBinaryString(prefix)

            if ip_binary[:length] == prefix_binary[:length]:
                if length > longest_match_len:
                    longest_match_len = length
                    best_route = min(self.rib[prefix_str], key=lambda x: len(x.path))
